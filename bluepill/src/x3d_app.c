...
{
  int data_offset = x3d_app_prepare_message(pstate, data, data_size, X3D_MESSAGE_TYPE_Pairing);
  if (data_offset < 0)
  {
    return data_offset;
  }

  X3D_HEADER_PAIRING_PAYLOAD *pairing_header_payload = (X3D_HEADER_PAIRING_PAYLOAD *)&data[data_offset];
  ...
  data_offset = x3d_app_finalize_header(pstate, data, data_size, sizeof(X3D_HEADER_PAIRING_PAYLOAD));
  if (data_offset < 0)
  {
    return data_offset;
  }

  X3D_PAIRING_PAYLOAD *pairing_payload = (X3D_PAIRING_PAYLOAD *)&data[data_offset];
  ...
  data_offset = x3d_app_finalize_message(pstate, data, data_size, sizeof(X3D_PAIRING_PAYLOAD));
  return data_offset;
}
